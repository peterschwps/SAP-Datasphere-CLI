...
if not os.path.isfile(SETTINGS_FILE):
    settings["URLs"] = {
        "DATASPHERE_TEST_URL": "https://example-test.eu10.hcs.cloud.sap", # System > Administration > Tenant Links: SAP Datasphere URL 
        "DATASPHERE_PROD_URL": "https://example-prod.eu10.hcs.cloud.sap"  # System > Administration > Tenant Links: SAP Datasphere URL
    }
    settings["Setup"] = {
        "URL_TO_USE": "DATASPHERE_PROD_URL",  # DATASPHERE_TEST_URL oder DATASPHERE_PROD_URL
        "AUTHENTICATION_METHOD": "REQUESTS"  # REQUESTS oder BROWSER
    }
    with open(SETTINGS_FILE, "w") as settings_file:
        settings.write(settings_file)
    logger.info(f"Settings Datei erstellt: {SETTINGS_FILE}")
...
class FileHandler:
    ...
    def __init__(self):
        ...
        for file in filter(lambda file: file["path"] == self.datasphere.ALL_PATHS["TASKS"],
                           self.datasphere.ALL_FILES.values()):
            self.create_file_if_not_exists(file["absolute_path"], file["columns"])

        # Prüfen, ob neue Task-Dateien erstellt wurden und Programm neu gestartet werden muss
        if self.quit_to_restart:
            logger.info("Es wurden neue Dateien erstellt. Bitte Programm erneut starten... ")
            sys.exit()

        # Alle Export- und Result-Dateien erstellen / überschreiben (außer .json Export-Dateien)
        for file in filter(lambda file: file["path"] in [self.datasphere.ALL_PATHS["EXPORTS"],
    ...
    def set_absolute_path(self, files: dict) -> None:
        for name, details in files.items():
            files[name]["absolute_path"] = os.path.join(PROJECT_PATH, details["path"], details["name"])
    ...
    def create_file_if_not_exists(self, abs_path: str, columns: Optional[list[str]]) -> None:
    ...
    def create_file(self, abs_path: str, columns: Optional[list[str]]) -> None:
        with open(abs_path, "w", encoding="utf-8") as file:
            if columns is not None:
